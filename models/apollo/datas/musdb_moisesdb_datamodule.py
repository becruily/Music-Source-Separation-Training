...
class MusdbMoisesdbDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        random_path = random.choice(self.data_path)
        
        #ori_wav = torchaudio.load(self.data_path[idx]+"/restored.wav")[0]
        #codec_wav = torchaudio.load(self.data_path[idx]+"/mixture.wav")[0]
        ori_wav = torchaudio.load(random_path+"/restored.wav")[0]
        codec_wav = torchaudio.load(random_path+"/mixture.wav")[0]
        
        start = random.randint(0, ori_wav.shape[-1] - self.segments)
        ori_wav = ori_wav[:, start:start+self.segments]
        codec_wav = codec_wav[:, start:start+self.segments]
